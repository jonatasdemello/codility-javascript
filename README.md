...
Based on the problem desciption: I will assume the input is "positive integer N",
...
To undestand it and make things more clear, I created the table below.
...
It is good to also add test cases to check conditions outside the scope or boundaries too.

Starting with the requirements from the problem description:
...
or using the "wacth mode" so the tests will run atuomatically (also using verbose to display more information):
...
In the test name, I added the converted binary number inside the backets to make it easier to understand: 9 decimal = [1001] binary. Like this: 'Given 9 [1001] should return 2.'
...
I may even go back and add more tests for other uses cases, but this is enough for now.
TDD is an incremental approach.
...
The reason for that is to see what the result of the converion is.
...
Assuming there are no trailing 0's so that all binary numbers will start with 1 (see table above, column 'bin') and
the converted binary number can be accessed as an array, I can use the index/array syntax:
...
Sometimes, when I am working with the algorithm logic,
I prefer to start with **Pseudocode**,
so I don't have to worry about the syntax or language construction rules.
